...
import { exportAllDialogs } from "../../kilocode/history/exportAllDialogs"
...
vi.mock("../../kilocode/history/exportAllDialogs", () => ({
	exportAllDialogs: vi.fn(),
}))
...
describe("exportAllDialogs command", () => {
	const outputChannel = {
		appendLine: vi.fn(),
	...
	it("returns without exporting when folder selection is cancelled", async () => {
		...
		await getCommandsMap({ context, outputChannel, provider: visibleProvider }).exportAllDialogs()
		...
		expect(exportAllDialogs).not.toHaveBeenCalled()
		expect(vscode.window.showInformationMessage).toHaveBeenCalledWith("Kilo text dialog export cancelled.")
	})

	it("passes the selected folder path to exportAllDialogs", async () => {
		vi.mocked(vscode.window.showOpenDialog).mockResolvedValue([{ fsPath: "/selected/export" }] as vscode.Uri[])
		vi.mocked(exportAllDialogs).mockResolvedValue({
			outputDir: "/selected/export",
			dialogsDir: "/selected/export/dialogs",
			taskHistoryPath: "/selected/export/dialogs_task_history.json",
			manifestPath: "/selected/export/dialogs_manifest.json",
		...
		expect(exportAllDialogs).toHaveBeenCalledWith(visibleProvider, { outputDir: "/selected/export" })
		expect(vscode.window.showInformationMessage).toHaveBeenCalledWith(
			"Exported 1, refreshed 1, skipped 0 of 2 Kilo text dialogs, failed 0. Output: /selected/export",
		)
	})
})
